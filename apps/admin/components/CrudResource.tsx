...
export type CrudField = { name: string; label: string; type?: "text" | "number" | "email" | "password" | "textarea" | "url" | "select" | "checkbox"; required?: boolean; options?: { value: string; label: string }[]; optionsEndpoint?: string };
...
export function CrudResource({ title, description, endpoint, fields, columns }: { title: string; description: string; endpoint: string; fields: CrudField[]; columns: { key: string; label: string }[] }) {
  ...
  function startCreate() { setEditing(null); setForm(Object.fromEntries(fields.map((field) => [field.name, field.type === "checkbox" ? false : ""]))); }
  function edit(row: CrudRow) { setEditing(row.id); setForm(Object.fromEntries(fields.map((field) => [field.name, row[field.name] as string | number | boolean ?? ""]))); }
  function change(name: string, value: string | number | boolean) { setForm((current) => ({ ...current, [name]: value })); }
